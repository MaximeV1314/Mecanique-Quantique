...
Lx = 100                               #longueur
Nb_x = 80                            #nombre de points de discrétisation en longueur
...
Ly = 100                              #longueur
Nb_y = 80                           #nombre de points de discrétisation en longueur
...
def Hamiltonien(V):
    ...
    vec = np.ones(Nb_x**2)
    H = (1/dx**2 + 1/dy**2 + V - B*Y.flatten()/dx + B*X.flatten()/dy) * np.diag(vec) - 1/(2*dx**2) * np.diag(vec[:-1], -1) - (1/(2*dx**2) - B*Y.flatten()/dx) * np.diag(vec[:-1], 1) -\
       (1/(2*dy**2) + B*X.flatten()/dy) * np.diag(vec[:-Nb_y], Nb_y) - 1/(2*dy**2) * np.diag(vec[:-Nb_y], -Nb_y)
